...
<div class="max-w-3xl mx-auto py-6">
    <h2 class="text-lg font-semibold mb-2">{{ $space->name }}</h2>

    <div class="mb-4"><strong>Building:</strong> @if($space->building)<a href="{{ route('buildings.show', $space->building) }}" class="text-blue-600">{{ $space->building->name }}</a>@else<span class="text-gray-500">No building</span>@endif</div>
    <div class="mb-4"><strong>Type:</strong> {{ $space->type }}</div>
    <div class="mb-4"><strong>Notes:</strong> <div class="mt-2 whitespace-pre-wrap">{{ $space->notes }}</div></div>

    <div class="mt-4">
        <a href="{{ route('spaces.index') }}">Back</a>
        @can('spaces.update')
            <a href="{{ route('spaces.edit', $space) }}" class="ml-4">Edit</a>
        @endcan
    </div>
    ...
    <div class="mt-8">
                        ...
                        @forelse($employments as $employment)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    @can('employments.update')
                                            <input type="hidden" name="return_to" value="{{ route('spaces.show', $space) }}" />
        ...
        @can('employments.create')
            <div class="mt-4">
                <button type="button" class="btn" onclick="window.dispatchEvent(new CustomEvent('open-modal',{detail:'employment-space-{{ $space->id }}'}))">Add Employment</button>
            </div>

            <x-modal name="employment-space-{{ $space->id }}" focusable>
                <div class="p-6">
                    ...
                    <form action="{{ route('employments.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            ...
                            <div class="flex gap-2 mt-4">
                                ...
                                <button type="button" class="ml-2 text-gray-600" onclick="window.dispatchEvent(new CustomEvent('close-modal',{detail:'employment-space-{{ $space->id }}'}))">Cancel</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <div class="mt-8">
        ...
        @can('addresses.view')
            <div class="bg-white shadow sm:rounded-lg mb-6">
                <div class="p-4">
                    @forelse($space->addresses as $address)
        ...
        @can('addresses.create')
            <div class="mt-4">
                <button type="button" class="btn" onclick="window.dispatchEvent(new CustomEvent('open-modal',{detail:'address-space-{{ $space->id }}'}))">Add Address</button>
            </div>

            <x-modal name="address-space-{{ $space->id }}" focusable>
                <div class="p-6">
                    <h4 class="text-lg font-medium mb-4">Add Address</h4>
                    <form action="{{ route('addresses.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="addressable_type" value="space" />
                        <input type="hidden" name="addressable_id" value="{{ $space->id }}" />
                        <input type="hidden" name="return_to" value="{{ route('spaces.show', $space) }}" />
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            ...
                            <div class="flex gap-2 mt-4">
                                <button class="btn">Add</button>
                                <button type="button" class="ml-2 text-gray-600" onclick="window.dispatchEvent(new CustomEvent('close-modal',{detail:'address-space-{{ $space->id }}'}))">Cancel</button>
                            </div>
                        </div>
                    </form>
                </div>
